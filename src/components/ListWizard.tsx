...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Loader2, PartyPopper, Search, Share2, Sparkles, Ticket, Users, Wand2 } from "lucide-react";
import type { BerthType, Gender, TravelClass } from "@/lib/types";
import type { PnrDetails } from "@/lib/pnr";
...
import { ClassBadge, BerthBadge } from "./Badges";
...
const STEPS = ["Journey", "Your seat", "What you want", "Review"];
...
export function ListWizard({ user, initial, demo, demoPnrs }: Props) {
  ...
  const [pnrState, setPnrState] = useState<"idle" | "looking" | "found" | "notfound">("idle");
  ...
  const lookup = async (p: string) => {
    ...
    try {
      ...
      if (d.found && d.details) {
        ...
        if (det.passengers.length === 1) applyPassenger(det, 0);
      } else {
        setPnrDetails(null);
        setPnrState("notfound");
      }
    } catch {
      setPnrState("notfound");
    }
  ...
  const applyPassenger = (det: PnrDetails, i: number) => {
    const p = det.passengers[i];
    setCoach(p.coach);
    setSeat(String(p.seatNo));
    if (p.berthType) {
    ...
    return (
      <div className="card animate-rise p-6 text-center sm:p-10">
        ...
        <p className="mx-auto mt-2 max-w-md text-slate-600">
          <span className="font-semibold">{normalizeCoach(coach)}-{seatNo}</span> on <span className="font-mono font-semibold">{trainNo}</span> for {formatDate(date)} is now visible to co-passengers.
          We&apos;ll notify you the moment someone proposes a swap.
        </p>
        {done.matchCount > 0 ? (
      ...
      <div className="card animate-rise p-5 sm:p-8" key={step}>
        ...
        {step === 0 && (
          <div className="space-y-6">
            <header>
              <h1 className="text-2xl font-extrabold tracking-tight">Which journey?</h1>
              <p className="mt-1 text-sm text-muted">Start with the PNR on your ticket. We use it only to verify the seat, it is never shown to anyone.</p>
            </header>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  id="pnr"
                  className="input pl-11 font-mono text-lg tracking-widest"
                  inputMode="numeric"
                ...
                />
                {pnrState === "looking" && <Loader2 className="absolute right-3.5 top-1/2 h-5 w-5 -translate-y-1/2 animate-spin text-brand-500" />}
                {pnrState === "found" && <CheckCircle2 className="absolute right-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-emerald-600" />}
              </div>
              {pnrState === "found" && pnrDetails && (
                <div className="mt-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm">
                  <p className="flex items-center gap-1.5 font-bold text-emerald-800">
                    <Wand2 className="h-4 w-4" /> Auto-filled from your PNR
                  </p>
                  <p className="mt-0.5 text-emerald-800/80">
                    {pnrDetails.trainNo} {pnrDetails.trainName} · {formatDate(pnrDetails.journeyDate)} · {pnrDetails.travelClass}
                  </p>
                  {pnrDetails.passengers.length > 1 && (
                    <div className="mt-2">
                      <p className="text-xs font-semibold text-emerald-900">Which seat do you want to list?</p>
                      <div className="mt-1.5 flex flex-wrap gap-2">
                        {pnrDetails.passengers.map((p, i) => (
                          <button
                            key={i}
                            type="button"
                            className={`chip ${coach === p.coach && seat === String(p.seatNo) ? "chip-on" : ""}`}
                            onClick={() => applyPassenger(pnrDetails, i)}
                          >
                            {p.coach}-{p.seatNo} {p.berthType ? `· ${BERTH_INFO[p.berthType].short}` : ""}
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              {pnrState === "notfound" && (
                <p className="hint">Couldn&apos;t auto-fetch this PNR. No problem, fill the details from your ticket below.</p>
              )}
              {demo && pnrState === "idle" && (
                <p className="hint">
                  Demo: try PNR{" "}
        ...
        {step === 1 && (
          <div className="space-y-6">
            <header>
              <h1 className="text-2xl font-extrabold tracking-tight">Your seat, as on the ticket</h1>
              <p className="mt-1 text-sm text-muted">Coach and seat are printed on your ticket, e.g. B2 / 23.</p>
            </header>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  placeholder={cls ? `e.g. ${CLASS_INFO[cls].coachPrefix}2` : "e.g. B2"}
                  value={coach}
                  maxLength={4}
                  onChange={(e) => setCoach(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder={cls ? `1 - ${CLASS_INFO[cls].maxSeat}` : "e.g. 23"}
                  value={seat}
                  min={1}
                  max={cls ? CLASS_INFO[cls].maxSeat : 120}
                  onChange={(e) => setSeat(e.target.value.replace(/\D/g, "").slice(0, 3))}
                />
              </div>
            </div>
        ...
        {step === 3 && cls && berth && (
          <div className="space-y-6">
            <header>
              ...
              <p className="mt-1 text-sm text-muted">This is how co-passengers will see your listing.</p>
            </header>
